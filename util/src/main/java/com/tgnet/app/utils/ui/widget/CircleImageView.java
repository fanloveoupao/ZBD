...
public class CircleImageView extends AbsRoundImageView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int width = Math.max(getMeasuredWidth(), getMeasuredHeight());//描绘成一个正方形
        setMeasuredDimension(width, width);
    }

    @Override
    protected void initRoundPath() {
        super.roundPath.reset();
        final int width = getWidth();
        final int height = getHeight();
        final float cx = width * 0.5f;
        final float cy = height * 0.5f;
        final float radius = Math.min(width, height) * 0.5f;
        super.roundPath.addCircle(cx, cy, radius, Path.Direction.CW);
    }

    @Override
    protected void initBorderPath() {
        borderPath.reset();
        final float halfBorderWidth = super.borderWidth * 0.5f;
        final int width = getWidth();
        final int height = getHeight();
        final float cx = width * 0.5f;
        final float cy = height * 0.5f;
