...
public class DividerItemDecoration extends RecyclerView.ItemDecoration {
     ...
     * @param right
     */
    public void setMargin(Context context, int left, int right) {
        this.marginLeft = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, left, context.getResources().getDisplayMetrics());
        this.marginRight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, right, context.getResources().getDisplayMetrics());
    ...
    private void drawLinearItemDivider(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (orientation == VERTICAL_LIST) {//画横线
                if (marginLeft > 0) left = marginLeft;
                else
                    left = child.getLeft() - params.leftMargin - marginLeft;
                if (marginRight > 0) right = child.getRight() + params.rightMargin - marginRight;
                else
                ...
                if (drawBorderTopAndBottom) {
                    ...
                    if (params.getViewLayoutPosition() == allChildCount - 2 && isHasFootView)
                        continue;
                    if (isFirstRaw(parent, params.getViewLayoutPosition(), spanCount) && isHasHeadView)
                        continue;
    ...
    private void drawHorizontalLine(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (drawBorderTopAndBottom) {
                ...
                int allChildCount = parent.getAdapter().getItemCount();
                if (params.getViewLayoutPosition() == allChildCount - 2 && isHasFootView && isHasHeadView)
                    continue;
                if (isFirstRaw(parent, params.getViewLayoutPosition(), spanCount) ||
                        isLastRaw(parent, params.getViewLayoutPosition(), spanCount, allChildCount)) {
                    continue;
                }
            }
            ...
            if (orientation == VERTICAL_LIST) {
                ...
                if (drawBorderLeftAndRight) {
                    if (marginLeft > 0) left = marginLeft;
                        ...
                        left = dividerWidth;
                    if (marginRight > 0) right = dividerWidth - marginRight;
                    else
                        right = dividerWidth;
                }
            } else if (orientation == HORIZONTAL_LIST) {
...
//                }
    ...
    public DividerItemDecoration setIsHasHeadView(boolean isHasHeadView) {
        this.isHasHeadView = isHasHeadView;
        return this;
    }

    public DividerItemDecoration setIsHasFootView(boolean isHasFootView) {
        this.isHasFootView = isHasFootView;
        return this;
    }
