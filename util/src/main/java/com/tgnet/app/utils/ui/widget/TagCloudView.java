...
public class TagCloudView extends ViewGroup {
    ...
    private  int mTagTextPaddingHor;
    private  int mTagTextPaddingVer;
    ...
    private static final int DEFAULT_TEXT_BORDER_VERTICAL = 0;

    private static final int DEFAULT_TAG_RESID = R.layout.item_tag;
    private static final int DEFAULT_RIGHT_IMAGE = R.mipmap.icon_right_arrow_white;
    ...
    public TagCloudView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTagTextPaddingHor = a.getDimensionPixelSize(R.styleable.TagCloudView_tcvTextPaddingHorizontal, DEFAULT_PADDING);
        mTagTextPaddingVer = a.getDimensionPixelSize(R.styleable.TagCloudView_tcvTextPaddingVertical, DEFAULT_PADDING);
        mTagColor = a.getResourceId(R.styleable.TagCloudView_tcvTextColor, R.color.white_ffffff);
        mTextColorState = getResources().getColorStateList(mTagColor);
    ...
    private void initSingleLineView(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mShowEndText) {
            ...
            if (mTagResId == DEFAULT_TAG_RESID) {
                ...
                endText.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTagSize);
//                endText.setTextColor(mTagColor);
                if (mTextColorState != null) {
                    endText.setTextColor(mTextColorState);
                }
            }
            @SuppressLint("DrawAllocation")
        ...
        if (tags != null && tags.size() > 0) {
            for (int i = 0; i < tags.size(); i++) {
                ...
                if (mTagResId == DEFAULT_TAG_RESID) {
                    ...
                    tagView.setPadding(mTagTextPaddingHor,mTagTextPaddingVer,mTagTextPaddingHor,mTagTextPaddingVer);
//                    tagView.setTextColor(mTagColor);
                    if (mTextColorState != null) {
                        tagView.setTextColor(mTextColorState);
                    }
                }
                LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
                tagView.setLayoutParams(layoutParams);
                tagView.setText(tags.get(i).toString());
    ...
    public void setTags(List<?> tagList, int tailLayoutId) {
        this.tags = tagList;
        this.removeAllViews();
        if (tags != null && tags.size() > 0) {
            for (int i = 0; i < tags.size(); i++) {
...
//                    tagView.setTextColor(mTagColor);
        ...
        View tailView = mInflater.inflate(tailLayoutId, null);
        tailView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if (onTagClickListener != null) {
                    onTagClickListener.onTailTagClick();
                }
            }
        });
        addView(tailView);
